...
CLASS aoc IMPLEMENTATION.
  ...
  METHOD show_interceptions.


    DATA shortest TYPE i VALUE 999999.
    ...
    IF sy-subrc = 0.
      WRITE: / 'interceptions exist!'.
      WRITE: / 'shortest interception distance:', shortest.
    ELSE.
      WRITE: / 'No interceptions!'.
    ENDIF.
  ...
  METHOD set.
    ...
    READ TABLE grid ASSIGNING FIELD-SYMBOL(<coordinate>)
    WITH KEY x = location-x
             y = location-y.
    IF sy-subrc = 0.
      ADD 1 TO <coordinate>-v.
    ELSE.
      INSERT VALUE #( x = location-x y = location-y v = 1 ) INTO TABLE grid.
    ENDIF.
